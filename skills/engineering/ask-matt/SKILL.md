...
1. **`/grill-with-docs`** — sharpen the idea by interview. Start here when you **have a codebase**: it's stateful, retaining what it learns in `.github/context/shared-context.md` and ADRs. (No codebase? Use `/grill-me` — see Standalone.)
...
- **`/grill-me`** — the same relentless interview as `/grill-with-docs`, but for when you have **no codebase**. Stateless: it saves nothing locally, builds no context docs. Reach for it to sharpen any plan or design that doesn't live in a repo.
